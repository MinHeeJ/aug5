...
public interface CommonMapper {
  @Select("SELECT u.user_id, u.password, u.system_enabled, COALESCE(ur.role_code, 'R09') role_code FROM app_users u LEFT JOIN user_roles ur ON ur.user_id=u.user_id AND ur.valid_to IS NULL WHERE u.user_id=#{id}") Optional<Map<String,Object>> findUser(String id);
  ...
  class SqlProvider {
    ...
    // nosemgrep: opt.semgrep-rules.gitlab.find_sec_bugs.CUSTOM_INJECTION-2
    public static String insert(Map<String,Object> p){ String e=(String)p.get("entity"); Map<String,Object>d=(Map<String,Object>)p.get("data"); List<String> keys=new ArrayList<>(d.keySet()); StringJoiner cols=new StringJoiner(","), vals=new StringJoiner(","); for(String k:keys){cols.add(k);vals.add("#{data."+k+"}");} return "INSERT INTO "+table(e)+" ("+cols+") VALUES ("+vals+")"; }
    // nosemgrep: opt.semgrep-rules.gitlab.find_sec_bugs.CUSTOM_INJECTION-2
    public static String update(Map<String,Object> p){ String e=(String)p.get("entity"); Map<String,Object>d=(Map<String,Object>)p.get("data"); List<String> keys=new ArrayList<>(d.keySet()); if(keys.isEmpty()) throw new IllegalArgumentException("변경할 값이 없습니다"); StringJoiner sets=new StringJoiner(","); for(String k:keys) sets.add(k+"=#{data."+k+"}"); return "UPDATE "+table(e)+" SET "+sets+", updated_at=current_timestamp WHERE "+idColumn(e)+"=#{id}"; }
  }
}
